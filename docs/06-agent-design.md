...
| `shadow_agent` 组合 | ✅ | `evaluation/shadow/`；日报 live 软跑 research DAG（启发式）取 Top-N |
...
**shadow_agent**：信号来自 `MarketBrief.stock_ranking`（`scores_from_brief`）；与 `shadow_factor` 同为等权 Top-N（默认 15）。日报 `run_daily_pipeline(live)` 默认软跑 `run_research_live(use_llm=False, include_knowledge=False)`；失败记 `no_agent_scores` / degraded，不阻断 baseline/factor。按组合幂等，升级后可追赶 agent 日记而不重复写旧组合。
