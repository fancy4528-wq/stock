"""财联社电报 via akshare ``stock_info_global_cls``."""
...
__all__ = ["ClsNewsCollector"]


class ClsNewsCollector(Collector):
    """Latest CLS telegraph flash items (rolling window from vendor)."""
    ...
    async def collect(self, target_date: date, **kwargs: Any) -> RawBatch:
        del kwargs  # no range filter from vendor
        df = await self._fetch()
        if df.is_empty():
            raise SourceUnavailableError("akshare.stock_info_global_cls returned empty")
        return self._archive.write(
            df,
            source=self.source,
            dataset=self.dataset,
            target_date=target_date,
            meta={"interface": "stock_info_global_cls"},
